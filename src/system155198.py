...
class BiasRatingSystem(RatingSystem):
    ...
    def calculate_user_bias(self, user):
        ...
        if user not in self.user_biases:            
            ...
            if n == 0:
                ...
                diff_sum = sum(r - g_mean - self.calculate_movie_bias(m) for m, r in ratings_dictionary.items())
    ...
    def _get_train_data(self):
        ...
        for user_id, user_obj in User.index.items():
            ...
            self.user_biases[user_id] = (sum(u_ratings) / len(u_ratings) - self.mean_global) if u_ratings else 0
            self.user_factors[user_id] = [random.uniform(-scale, scale) for _ in range(self.k)]
            ...
            for movie_ref, rating in user_obj.ratings.items():
                ...
                if m_id not in self.movie_factors:
                    self.movie_factors[m_id] = [random.uniform(-scale, scale) for _ in range(self.k)]
        ...
        for t_u_id, t_m_id in test_users:
            if t_u_id not in self.user_factors:
                self.user_factors[t_u_id] = [random.uniform(-scale, scale) for _ in range(self.k)]
                self.user_biases[t_u_id] = 0
            if t_m_id not in self.movie_factors:
                self.movie_factors[t_m_id] = [random.uniform(-scale, scale) for _ in range(self.k)]
    ...
    def _train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for u_id, m_id, r in data:
                ...
                prediction = self.mean_global + self.user_biases[u_id] + self.movie_biases[m_id] + interaction
                
                error = r - prediction
            
                self.user_biases[u_id] += lr * (error - self.lamb_b * self.user_biases[u_id])
                self.movie_biases[m_id] += lr * (error - self.lamb_b * self.movie_biases[m_id])
